...
const MyItems = () => {
  ...
  useEffect(() => {
    const url = `https://warehouse-management-ssr.onrender.com/myitems?email=${email}`;
  ...
  const deleteFromMyItems = async (id) => {
    ...
    if (deleteMyItem) {
      const url = `https://warehouse-management-ssr.onrender.com/myitems/${id}`;
          ...
          <tbody>
            {items.map((item, index) => (
              <tr key={item._id}>
                ...
                <td>{item.price} $</td>
                <td>
                  {item.quantity} <small>(piece/gm/kg)</small>{" "}
                </td>
                <td>
                  <button
                    className="btn btn-light border-0"
                    onClick={() => deleteFromMyItems(item._id)}
                  >
